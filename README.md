...
| `backfill_primas.py` | Script de ejecución única para recuperar historial de primas diarias de COCOA directamente del endpoint de actividad de Capital.com |
...
| v19 | MongoDB: activo `USCOCOA` actualizado en colección `activos` — `configuracion` cambiada de vacío a `"Cacao US"` y `fechaInicioConfiguracion` establecida en `2026-05-08`. Trades abiertos antes de esa fecha quedan sin configuración (string vacío), según la lógica existente en Power BI. |
| v20 | Fix renombramiento de activos: a partir del 01-06-2026 Capital.com renombró `Gold` → `XAUUSD`, `Silver` → `XAGUSD` y `COCOA US` → `USCOCOA`. Se agrega diccionario `NOMBRE_CANONICO` y función `normalizar_instrumento()` en `capital_downloader.py` que traduce los nombres nuevos al nombre canónico histórico antes de insertar en MongoDB, tanto en `operaciones[instrumentName]` como en `trades[details_marketName]`. Se agrega función `migrar_nombres_canonicos()` que corrige registros ya existentes en cada ejecución del Lambda. Script auxiliar `migrar_nombres.py` para ejecución local de la migración inicial. |
| v21 | Fix prima diaria COCOA: Capital.com aplica un ajuste de prima diaria a las 13:30 para instrumentos de tipo forward (como COCOA US) que no aparece en el endpoint de transacciones sino en la actividad como registros `type=POSITION`, `source=SYSTEM`. El costo real es `-(details_level × details_size)`. Se agregan funciones `crear_prima_diaria_doc()` que convierte estos registros en entradas `SWAP` con `note="Daily premium"` para `operaciones`, `migrar_primas_existentes()` que hace backfill desde `trades` en cada ejecución, y detección inline en el loop de trades para capturar primas futuras automáticamente. |
| v22 | Fix backfill primas: `migrar_primas_existentes()` era incorrecta — intentaba recuperar primas desde la colección `trades`, pero todos los registros POSITION/SYSTEM del mismo trade comparten el mismo `dealId` y la deduplicación dejaba solo 1 entrada en `trades`, no una por día. Función eliminada. La detección inline en el loop de trades (v21) ya captura correctamente las primas futuras. Para el historial se crea `backfill_primas.py`: script de ejecución única que consulta directamente el endpoint `/history/activity` de Capital.com día a día y hace upsert en `operaciones` usando `reference = daily_prem_{dealId}_{fecha}` como clave. |
